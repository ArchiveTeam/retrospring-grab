...
LUA_SHA1 = get_hash(os.path.join(CWD, 'retrospring.lua'))
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'account':
                ...
                wget_args.extend(['--warc-header', 'retrospring-question: '+post])
                wget_args.append('https://retrospring.net/@{}/q/{}'.format(user, post))
            elif item_type == 'answer':
                user, post = item_value.split(':')
                wget_args.extend(['--warc-header', 'retrospring-answer: '+post])
                wget_args.append('https://retrospring.net/@{}/a/{}'.format(user, post))
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'retrospring-asset: '+url])
